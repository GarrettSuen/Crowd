...
/**
 * 定制化DelegatingFilterProxy
 * 原因：
 *      1.SSM架构中存在Spring的IOC容器和SpringMVC的IOC容器
 *      2.DelegatingFilterProxy由SpringMVC的IOC容器所加载，故DelegatingFilterProxy所需要的SpringSecurityFilterChain Bean存在于该IOC容器中
 *      3.在初始化FilterBean中会寻找根IOC（SpringIOC）从IOC容器中加载SpringSecurityFilterChain这个bean
 *      4.故在初始化时出现无法找到SpringSecurityFilterChain这个bean的异常
 * 
 * 目的：
 *      使其在处理第一次请求时在进行初始化DelegatingFilterProxy的操作，并且从SpringMVC中加载SpringSecurityFilterChain这个bean
 *      从而避免出现初始化时无法加载到SpringSecurityFilterChain这个bean的异常出现
 *      
 * 定制操作：
 *      1.取消初始化寻找IOC容器从而初始化                                   line number：259-262
 *      2.利用懒加载在处理第一次请求中直接查找SpringMVC的IOC容器再进行初始化   line number：268-275
 */
        ...
        if (delegateToUse == null) {
            synchronized (this.delegateMonitor) {
                ...
                if (delegateToUse == null) {
                    // 1.取消查找根IOC容器
//                    WebApplicationContext wac = findWebApplicationContext();
                    // 2.获取ServletContent对象
                    ServletContext sc = this.getServletContext();
                    // 3.准备SpringMVC的IOC属性名称，用于从sc中获取到SpringMVC的IOC容器
